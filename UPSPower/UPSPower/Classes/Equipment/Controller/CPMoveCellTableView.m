...
@implementation CPMoveCellTableView
    ...
    switch (longPressState) {
        case UIGestureRecognizerStateBegan:{  //手势开始，对被选中cell截图，隐藏原cell
            ...
            if (_originalIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(tableView:cellReadyToMoveAtIndexPath:)]) {
                 [self.delegate tableView:self cellReadyToMoveAtIndexPath:_originalIndexPath];
                }
            }
            break;
        }
        case UIGestureRecognizerStateChanged:{//点击位置移动，判断手指按住位置是否进入其它indexPath范围，若进入则更新数据源并移动cell
            ...
            if ([self.delegate respondsToSelector:@selector(cellDidEndMovingInTableView:)]) {
                [self.delegate cellIsMovingInTableView:self];
            }
...
- (void)updateDataSource{
    ...
    if ([self nestedArrayCheck:tempArray]) {//是嵌套数组
        ...
        tempArray[_originalIndexPath.section]= [self moveObjectInMutableArray:tempArray[_originalIndexPath.section] fromIndex:_originalIndexPath.row toIndex:_relocatedIndexPath.row];
        }else{                                                          //不在同一个section内
            id originalObj = tempArray[_originalIndexPath.section][_originalIndexPath.item];
            
            NSMutableArray *relocatedArr=[NSMutableArray arrayWithArray:tempArray[_relocatedIndexPath.section]];
            [relocatedArr insertObject:originalObj atIndex:_relocatedIndexPath.item];
            tempArray[_relocatedIndexPath.section]=relocatedArr;
            //[tempArray[_relocatedIndexPath.section] insertObject:originalObj atIndex:_relocatedIndexPath.item];
            
            NSMutableArray *originalArr=[NSMutableArray arrayWithArray:tempArray[_originalIndexPath.section]];
            [originalArr removeObjectAtIndex:_originalIndexPath.item];
            tempArray[_originalIndexPath.section]=originalArr;
             // [tempArray[_originalIndexPath.section] removeObjectAtIndex:_originalIndexPath.item];
        }
    }else{           //不是嵌套数组
             tempArray = [self moveObjectInMutableArray:tempArray fromIndex:_originalIndexPath.row toIndex:_relocatedIndexPath.row];
    }
    //将新数组传出外部以更改数据源
...
- (NSMutableArray *)moveObjectInMutableArray:(NSMutableArray *)array fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex{
    ...
    NSMutableArray *mrr=[NSMutableArray arrayWithArray:array];
    if (fromIndex < toIndex) {
        for (NSInteger i = fromIndex; i < toIndex; i ++) {
            [mrr exchangeObjectAtIndex:i withObjectAtIndex:i + 1];
        }
    }else{
        for (NSInteger i = fromIndex; i > toIndex; i --) {
            [mrr exchangeObjectAtIndex:i withObjectAtIndex:i - 1];
